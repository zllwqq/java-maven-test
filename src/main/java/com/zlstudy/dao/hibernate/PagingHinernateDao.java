...
import com.zlstudy.dao.hibernate.PropertyFilter.MatchType;
...
* @author 
*/
public class PagingHinernateDao<T, PK extends Serializable> extends SimpleHibernateDao<T, PK>{
	/**
	 * 用于Dao层子类的构造函数.
	 ...
	 */
	public PagingHinernateDao(final SessionFactory sessionFactory, final Class<T> entityClass) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <X> Page<X> findPage(Class<X> clasz,final Page<X> page, final String sql, final Object... values) {
		Assert.notNull(page, "page不能为空");
	...
	protected long countSqlResult(final String sql, final Object... values) {
		...
		//Hibernate Query执行count，返回结果类型为Long，而SQLQuery执行count返回类型为BigInter？？？？
		BigInteger count = (BigInteger)createSQLQuery(countSql, values).uniqueResult();
	...
	private String prepareCountSql(String sql) {
		...
		if (sql.indexOf("union") > -1) {
			//mysql from子句设置别名
			countSql = "select count(*) from ("+StringUtils.substringBefore(sql, "order by")+") a";
		} else {
			//select子句与order by子句会影响count查询,进行简单的排除.
			String fromHql = "from " + StringUtils.substringAfter(sql, "from");
			fromHql = StringUtils.substringBefore(fromHql, "order by");
			countSql = "select count(*) " + fromHql;
		}
		return countSql;
	}
